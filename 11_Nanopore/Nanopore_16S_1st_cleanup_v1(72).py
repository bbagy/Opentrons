def get_values(*names):
    import json
    _all_values = json.loads("""{"p300_multi_mount":"left","number_of_samples":72,
    "volume_of_beads":50,"bead_incubation_time_in_minutes":1,"etoh_inc":4,"drying_time_in_minutes":1,
...
metadata = {
    'protocolName': 'Nanopre 16S 1st bead clean up (50µl with 72 samples)',
...
resuming.')
    ...
    for a, m, p in zip(amp_samples,mag_samples, parking_spots):
        ...
        m300.transfer(volume_of_beads, beads, a.bottom(),air_gap=5, new_tip='never')
        m300.blow_out(a.top(-2))
        for _ in range(3):
            m300.flow_rate.aspirate = 100
            m300.flow_rate.dispense = 150
            m300.aspirate(volume_of_beads+30, a.bottom(-3))
            m300.dispense(volume_of_beads+30, a.center())
        m300.blow_out(a.top(-2))
        
        m300.flow_rate.aspirate = 20
        m300.transfer(volume_of_beads+100,
                      a.bottom(-4.6), m,
                      new_tip='never')
