...
def run(protocol: protocol_api.ProtocolContext):
    ...
    sample_plate_final  = protocol.load_labware('biorad_96_wellplate_200ul_pcr','10')
    ...
    # pipette
    p300    = protocol.load_instrument('p300_multi_gen2', 'left', tip_racks=[tiprack_200, tiprack_200_1,tiprack_200_2,tiprack_200_3,tiprack_200_4,tiprack_200_5])
    ...
    column_1_list = column_list[:COLUMNS]
    column_2_list = column_list[:COLUMNS]
    column_3_list = column_list[:COLUMNS]
    column_4_list = column_list[:COLUMNS]
    ...
    # column_4_list = ['A1','A2','A3','A4','A5','A6','A7','A8','A9','A10','A11','A12']
    ...
    def p300_reuse_tip(loop):
        ...
        if TIPREUSE == 'YES':
            ...
            if (p300_tips-COLUMNS+loop) <=11:
                p300.pick_up_tip(tiprack_200.wells()[(p300_tips-COLUMNS+loop)*8])
                protocol.comment('tiprack_200 : '+str((p300_tips-COLUMNS+loop)*8))
            elif (p300_tips-COLUMNS+loop) <=23:
                p300.pick_up_tip(tiprack_200_1.wells()[(p300_tips-12-COLUMNS+loop)*8])
                protocol.comment('tiprack_200_1 : '+str((p300_tips-12-COLUMNS+loop)*8))
            elif (p300_tips-COLUMNS+loop) <=35:
                p300.pick_up_tip(tiprack_200_2.wells()[(p300_tips-24-COLUMNS+loop)*8])
                protocol.comment('tiprack_200_2 : '+str((p300_tips-24-COLUMNS+loop)*8))
            elif (p300_tips-COLUMNS+loop) <=47:
                p300.pick_up_tip(tiprack_200_3.wells()[(p300_tips-36-COLUMNS+loop)*8])
                protocol.comment('tiprack_200_3 : '+str((p300_tips-36-COLUMNS+loop)*8))
            elif (p300_tips-COLUMNS+loop) <=59:
                p300.pick_up_tip(tiprack_200_4.wells()[(p300_tips-48-COLUMNS+loop)*8])
                protocol.comment('tiprack_200_4 : '+str((p300_tips-48-COLUMNS+loop)*8))
            elif (p300_tips-COLUMNS+loop) <=71:
                p300.pick_up_tip(tiprack_200_5.wells()[(p300_tips-60-COLUMNS+loop)*8])
                protocol.comment('tiprack_200_5 : '+str((p300_tips-60-COLUMNS+loop)*8))
    ...
    if STEP_FRERAT == 1:
        ...
        for loop, X in enumerate(column_1_list):
            p300_pick_up_tip()
            p300.aspirate(FRERATVol, FRERAT.bottom())
            p300.dispense(FRERATVol, sample_plate_thermo.wells_by_name()[X].bottom())
            p300.move_to(sample_plate_thermo[X].bottom())
            p300.mix(FRERATMixRep,FRERATMixVol)
    ...
    if STEP_LIG == 1:
        ...
        LIGVol = 30
        LIGMixRep = 10 if DRYRUN == 'NO' else 1
        LIGMixVol = 55
        LIGCOUNT = 0
        for loop, X in enumerate(column_1_list):
            p300_pick_up_tip()
    ...
    if STEP_POSTLIG == 1:
        ...
        AMPureVol = 48
        AMPureMixRep = 10 if DRYRUN == 'NO' else 1
        AMPureMixVol = 90
        ...
        protocol.comment('--> Multi Dispense ETOH Wash #1')
        ETOHMaxVol = 90
        NumDispenses = 2
        ETOHCount = 0
        EtOHMultiDispense = math.floor(COLUMNS / NumDispenses)
        EtOHRemainder = (COLUMNS % NumDispenses)
        EtOHMultiDispVol = ETOHMaxVol * NumDispenses * 1.00
        #Increase the 1.00 multiplyer to manage overage aspirated for MultiEtOH dispense
        ETOHRemainderDispVol = EtOHRemainder * ETOHMaxVol
        Counter = EtOHRemainder
        
        p300_pick_up_tip()
        for loop, X in enumerate(column_1_list): 
            if EtOHMultiDispense > 0:
                if NumDispenses == 2:
                    if ETOHCount <= 5:
                        p300.aspirate(EtOHMultiDispVol, EtOH_1.bottom())
                    else:
                        p300.aspirate(EtOHMultiDispVol, EtOH_2.bottom())
                    p300.move_to(sample_plate_mag[X].top(z=2))
                    p300.dispense(ETOHMaxVol, rate=0.5)
                    NumDispenses = 1
                    ETOHCount += 2
                    EtOHMultiDispense -= 1
                else:
                    ...
                    p300.dispense(ETOHMaxVol, rate=0.5)
                    NumDispenses += 1
            elif EtOHMultiDispense == 0:
                p300.move_to(sample_plate_mag[X].top(z=2))
                p300.dispense(ETOHMaxVol, rate=0.5)
                EtOHMultiDispense -= 1
            else:
                if Counter == EtOHRemainder:
                    if ETOHCount <= 5:
                        p300.aspirate(ETOHRemainderDispVol, EtOH_1.bottom())
                    else:
                        p300.aspirate(ETOHRemainderDispVol, EtOH_2.bottom())
                    p300.move_to(sample_plate_mag[X].top(z=2))
                    p300.dispense(ETOHMaxVol, rate=0.5)
                    Counter -= 1
                    ETOHCount += 1
        ...
        protocol.comment('--> Remove ETOH Wash #1 and Multi Dispense ETOH Wash #2')
        ETOHMaxVol = 90
        NumDispenses = 2
        ETOHCount = 0
        DryCount = 0
        ...
        EtOHMultiDispense = math.floor(COLUMNS / NumDispenses)
        EtOHRemainder = (COLUMNS % NumDispenses)
        EtOHMultiDispVol = ETOHMaxVol * NumDispenses
        ETOHRemainderDispVol = EtOHRemainder * ETOHMaxVol
        Counter = EtOHRemainder
        protocol.comment(str(EtOHMultiDispVol))
        protocol.comment(str(ETOHRemainderDispVol))
        protocol.comment(str(EtOHMultiDispense))
        protocol.comment(str(EtOHRemainder))
        
        #Building DryCounter List
        DryCounter = []
        for i in range(math.floor(COLUMNS / DryColumns)):
            DryCounter.append(DryColumns - 1)
        if COLUMNS % DryColumns > 0:
            DryCounter.append((COLUMNS % DryColumns) - 1)
            pass
       
        #Starting sup removes
        for loop, X in enumerate(column_2_list):
            Num = 0
            if DryCount < DryCounter[Num]:
                p300_reuse_tip(loop)
                ...
                p300_move_to(X,'p300_bead_side')
                protocol.delay(minutes=0.1)
                p300.aspirate(140-ETOHMaxVol, rate=0.25)
                p300.default_speed = 400
                p300_dispose(140)
                p300.drop_tip() if DRYRUN == 'NO' else p300.return_tip()
                DryCount += 1
                ...
                DryCount += 1
                DryList.append(X)
            #EtOH Multi Dispense after indicated number of DryColumns 
                p300_pick_up_tip()
                for loop, X in enumerate(DryList): 
                    if EtOHMultiDispense > 0:
                        if NumDispenses == 2:
                            if ETOHCount <= 5:
                                p300.aspirate(EtOHMultiDispVol, EtOH_1.bottom())
        ...
        #EtOH Multidispense for last columns
        p300_pick_up_tip()
        for loop, X in enumerate(DryList): 
            if EtOHMultiDispense > 0:
                if NumDispenses == 2:
    ...
    protocol.comment('--> Remove ETOH Wash #2 and Multi Dispense RSB')
    RSBVol = 21 if STEP_POSTLIGSS == 0 else 50
    ETOHMaxVol = 90
    NumDispenses = 4
    RSBCount = 0
    DryCount = 0
    DryList = []
    DryColumns = 4
    RSBMultiDispense = math.floor(COLUMNS / NumDispenses)
    RSBRemainder = (COLUMNS % NumDispenses)
    RSBMultiDispVol = RSBVol * NumDispenses * 1.05
    RSBRemainderDispVol = RSBRemainder * RSBVol
    Counter = RSBRemainder
    protocol.comment(str(RSBMultiDispVol))
    protocol.comment(str(RSBRemainderDispVol))
    protocol.comment(str(RSBMultiDispense))
    protocol.comment(str(RSBRemainder))
    ...
    for loop, X in enumerate(column_2_list):
        ...
        if DryCount < DryCounter[Num]:
            ...
            DryCount += 1
            DryList.append(X)
        #RSB Multi Dispense after indicated number of DryColumns 
            p300_pick_up_tip()
            for loop, X in enumerate(DryList): 
                if RSBMultiDispense > 0:
                    if NumDispenses == 4:
                        p300.aspirate(RSBMultiDispVol, RSB.bottom())
                        p300.move_to(sample_plate_mag[X].top(z=2))
                        p300.dispense(RSBVol, rate=0.5)
                        NumDispenses = 1
                        EtOHMultiDispense -= 1
                    else:
                        p300.move_to(sample_plate_mag[X].top(z=2))
    ...
    #RSB Multidispense for last columns
    p300_pick_up_tip()
    for loop, X in enumerate(DryList): 
        if EtOHMultiDispense > 0:
            if NumDispenses == 4:
                ...
                p300.dispense(RSBVol, rate=0.5)
                NumDispenses += 1
        elif RSBMultiDispense == 0:
            p300.move_to(sample_plate_mag[X].top(z=2))
            p300.dispense(RSBVol, rate=0.5)
            RSBMultiDispense -= 1
        else:
            if Counter == RSBRemainder:
                p300.aspirate(RSBMultiDispVol, RSB.bottom())
                p300.move_to(sample_plate_mag[X].top(z=2))
                p300.dispense(RSBVol, rate=0.5)
                Counter -= 1
...
#             protocol.delay(minutes=0.5)
    ...
    for loop, X in enumerate(column_1_list):
        ...
        if STEP_POSTLIGSS == 0:
            #Update dispense location to be sample_plate_final instead of mag
