def get_values(*names):
    ...
    "volume_of_beads":18,"bead_incubation_time_in_minutes":1,"etoh_inc":2,"drying_time_in_minutes":1,
    "vol_etoh":80,"mix_etoh":false,"volume_EB_in_ul":52,"elution_inc":1,"volume_final_elution_in_ul":50,
...
metadata = {
    'protocolName': '16S 1st bead clean up',
...
def run(ctx):
    ...
    amp_plate = ctx.load_labware(
        'heekuk_96_wellplate_with_rack_200ul', '3')
    ...
    tips300 = [
        ctx.load_labware('opentrons_96_tiprack_300ul', slot) #opentrons_96_filtertiprack_200ul
        for slot in ['6', '8', '9']]
...
resuming.')
    ...
    for a, m, p in zip(amp_samples,mag_samples, parking_spots):
        ...
        m300.transfer(volume_of_beads, beads, a.bottom(), air_gap=2, new_tip='never')
        m300.blow_out(a.top(-2))
        for _ in range(5):
            m300.flow_rate.aspirate = 100
            m300.flow_rate.dispense = 150
            m300.aspirate(volume_of_beads+17, a.bottom(-3))
            m300.dispense(volume_of_beads+17, a.center())
        m300.blow_out(a.top(-2))
        
        m300.flow_rate.aspirate = 20
        m300.transfer(volume_of_beads+50,
                      a.bottom(-4.6), m,
                      new_tip='never')
...
on magnet for ' + str(etoh_inc) + ' minutes.')
    ...
    for m, p, w in zip(mag_samples, parking_spots, waste):
        pick_up(m300, p)
        m300.aspirate(60, m.bottom(0.5))
        m300.air_gap(10)
        m300.dispense(70, w, rate=0.7)
        m300.blow_out(w)
        m300.air_gap(20)
        drop(m300, p)
...
' + str(drying_time_in_minutes) + ' minutes.')
                ...
                # custom distribution
                col_per_asp = math.floor(210/volume_EB_in_ul)# it was 180
    ...
    for m in mag_samples:
        pick_up(m300)
        m300.mix(6, 0.8*volume_EB_in_ul, m)
        m300.blow_out(m.top())
        drop(m300)
