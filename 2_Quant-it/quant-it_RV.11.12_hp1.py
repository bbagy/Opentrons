...
metadata = {
    'protocolName': 'Quant-iT dsDNA Broad-Range Assay Kit(reservoir 11 and 12)',
...
def run(protocol):
    ...
    for well in wells6:
        p300.transfer(200, res['A11'], aplate[well].top(), new_tip='never')
        p300.air_gap(5)
        p300.blow_out(aplate[well].top())
        
    wells12 = ['A'+str(i) for i in range(7, 13)]
    for well in wells12:
        p300.transfer(200, res['A12'], aplate[well].top(), new_tip='never')
        p300.air_gap(5)
        p300.blow_out(aplate[well].top())
