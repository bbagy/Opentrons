...
# Setup the window
root = Tk()
root.title("(Adding DNA) Dilution With Heekuk!!")
...
def browse_file():
    ...
    dest_file_path.set(os.path.splitext(filename)[0] + '_ot2_DNA_water.py')
...
# metadata
metadata = {
    'protocolName': '(Delete this protocol after use) Water and DNA Transfer with CSV File',
...
def run(protocol):
    ...
    plate_dna = protocol.load_labware('nest_96_wellplate_100ul_pcr_full_skirt', '3')  # Labware for DNA volumes
    ...
    for line in transfer:
        ...
        well = line[3]
        well_dna = line[0]  # Assume that the sample names match the well names in the DNA plate
        ...
        p300.blow_out(plate.wells_by_name()[well].bottom())
        total_volume -= vol_water
        tip_height = total_volume / 200  # Update tip height based on remaining volume


        # DNA transfer
        # p300.return_tip()  # Return the tip to the rack
        # p300.pick_up_tip()  # Pick up a new tip for the DNA transfer
        vol_dna = float(line[1])  # Get the DNA volume from column 1
        p300.transfer(vol_dna, plate_dna.wells_by_name()[well_dna], plate.wells_by_name()[well].bottom(z=3), new_tip='never')
        p300.blow_out(plate.wells_by_name()[well].bottom())
        p300.drop_tip()  # Drop the tip after the DNA transfer
...
def compile():
    ...
    try:
        ...
        with open(source_file, 'r') as file:
            ...
            for row in reader:
                try:
                    ...
                    dna_vol = float(dna_vol)
                    csv_data.append([sample, dna_vol, water_vol, well])

                    total_volume += water_vol
                    max_volume = max(max_volume, water_vol)
                except ValueError as e:
                    print(f"Skipping row due to error: {row}, error: {str(e)}")
        ...
        # If an error occurs, open a new window and display the error message.
        error_window = Toplevel(root)
        error_window.title("Error")
        error_label = Label(error_window, text=str(e))
        error_label.pack()
        ok_button = Button(error_window, text="OK", command=error_window.destroy)
        ok_button.pack()
...
# Is the the code transfering 96 water to the certain location and transfering DNA?
